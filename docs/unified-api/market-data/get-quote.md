...
지정한 종목의 최근 체결가 스냅샷을 가져옵니다. 단일 호출로 마지막 체결가, 시가/고가/저가, 전일 대비 변동, 누적 거래량을 받을 수 있어 대시보드와 알림용 가벼운 폴링에 적합합니다. Toss Invest provider는 여러 종목을 한 번에 조회하는 `get_quotes`도 제공합니다. 호가창 깊이가 필요하면 [`get_orderbook`](get-orderbook.md)을 사용하세요.

## At a glance

| 항목 | 값 |
|---|---|
| 인증 필요 | 예 (provider 자격증명) |
| 데이터 타입 | 단일 시점 시세 스냅샷 |
...
async def get_quotes(symbols: Iterable[str | InstrumentRef], /) -> QuotesResponse: ...
```
...
`get_quotes`는 현재 Toss Invest provider에서 지원하며, 최대 200개 심볼을 한 번에 조회합니다.
...
- **가격은 `Decimal`**입니다.
- **Toss Invest batch quote**: `/api/v1/prices?symbols=...`를 사용하며, 최대 200개 심볼을 콤마 구분으로 조회합니다.
